  ...
  <div class="container-fixed flex items-center justify-between flex-wrap gap-3">
    <div class="flex items-center flex-wrap gap-1 lg:gap-5">
      ...
      <div class="flex items-center gap-1 text-sm font-normal">
        ...
        <span class="text-gray-900">Fornecedores</span>
      </div>
    </div>
    <div class="flex items-center flex-wrap gap-1.5 lg:gap-3.5">
      <a class="btn btn-sm btn-primary" href="<?= base_url('Fornecedores/criar') ?>">
    ...
    <div class="card card-grid min-w-full">
      <div class="card-header flex flex-col lg:flex-row gap-4 lg:items-center lg:justify-between">
        <h3 class="card-title font-medium text-sm">
          Mostrando <?= count($fornecedores) ?> fornecedor(es)
        </h3>
        <div class="flex flex-col lg:flex-row gap-3 w-full lg:w-auto lg:items-center">
          ...
          <div class="flex flex-1 lg:flex-initial lg:min-w-[280px]">
            <label class="input input-sm w-full">
              ...
              <input id="busca" placeholder="Buscar fornecedores..." type="text" value="<?= esc($filtros['busca'] ?? '') ?>">
            </label>
          </div>
          <!-- Filtros -->
        ...
        <div data-datatable="true" data-datatable-page-size="10">
              ...
              <thead>
                <tr>
                  ...
                  <th class="min-w-[250px]">
                    <span class="sort">
                      <span class="sort-label font-normal text-gray-700">Razão Social</span>
                      <span class="sort-icon"></span>
                    </span>
                  </th>
                  <th class="min-w-[200px]">
                    <span class="sort">
                      <span class="sort-label font-normal text-gray-700">Nome Fantasia</span>
                      <span class="sort-icon"></span>
                    </span>
                  </th>
                  <th class="min-w-[150px]">
                    <span class="sort">
                      <span class="sort-label font-normal text-gray-700">CNPJ/CPF</span>
                      <span class="sort-icon"></span>
                    </span>
                  </th>
                  <th class="min-w-[150px]">
                  ...
                  <?php foreach ($fornecedores as $fornecedor): ?>
                    <tr>
                      <td class="text-center">
                        <input class="checkbox checkbox-sm" data-datatable-row-check="true" type="checkbox" value="<?= $fornecedor['id_fornecedor'] ?>">
                      </td>
                      <td>
                        <div class="flex items-center gap-2.5">
                          ...
                          <div class="flex flex-col">
                            <a class="text-sm font-medium text-gray-900 hover:text-primary-active mb-px" href="<?= base_url('Fornecedores/editar/' . $fornecedor['id_fornecedor']) ?>">
                      ...
                      <td class="text-gray-800 font-medium">
                        <?php 
                        $doc = $fornecedor['cnpj'] ?? $fornecedor['cpf'] ?? '';
                        if ($doc) {
                          if (strlen($doc) == 14) {
                            // CNPJ
                      ...
                      <td class="text-gray-800 font-medium">
                        <?= esc($fornecedor['telefone'] ?? $fornecedor['celular'] ?? '-') ?>
                      </td>
                      <td>
                        <?php if ($fornecedor['email']): ?>
                          <a class="text-sm text-gray-700 font-normal hover:text-primary-active" href="mailto:<?= esc($fornecedor['email']) ?>">
                      ...
                      <td>
                        <div class="flex justify-center gap-2">
                          <a class="btn btn-sm btn-icon btn-light" href="<?= base_url('Fornecedores/editar/' . $fornecedor['id_fornecedor']) ?>" title="Editar">
                            <i class="ki-filled ki-notepad-edit"></i>
                          </a>
                          <a class="btn btn-sm btn-icon btn-light" href="#" 
                             onclick="return confirmarExclusao('<?= base_url('Fornecedores/excluir/' . $fornecedor['id_fornecedor']) ?>', 'Excluir Fornecedor?', 'Tem certeza que deseja excluir este fornecedor? Esta ação não pode ser desfeita!')" title="Excluir">
          ...
          <div class="card-footer justify-center md:justify-between flex-col md:flex-row gap-5 text-gray-600 text-2sm font-medium">
            ...
            <div class="flex items-center gap-4 order-1 md:order-2">
              <span data-datatable-info="true">1-<?= count($fornecedores) ?> de <?= count($fornecedores) ?></span>
            </div>
          </div>
        </div>
      </div>
...
function aplicarFiltros() {
  ...
  window.location.href = '<?= base_url('Fornecedores') ?>?' + params.toString();
}
