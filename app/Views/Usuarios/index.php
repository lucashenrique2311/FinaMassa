  ...
  <div class="container-fixed flex items-center justify-between flex-wrap gap-3">
    <div class="flex items-center flex-wrap gap-1 lg:gap-5">
      ...
      <div class="flex items-center gap-1 text-sm font-normal">
        ...
        <span class="text-gray-900">Usuários</span>
      </div>
    </div>
    <div class="flex items-center flex-wrap gap-1.5 lg:gap-3.5">
      <a class="btn btn-sm btn-primary" href="<?= base_url('Usuarios/criar') ?>">
    ...
    <div class="card card-grid min-w-full">
      <div class="card-header flex flex-col lg:flex-row gap-4 lg:items-center lg:justify-between">
        <h3 class="card-title font-medium text-sm">
          Mostrando <?= count($usuarios) ?> usuário(s)
        </h3>
        <div class="flex flex-col lg:flex-row gap-3 w-full lg:w-auto lg:items-center">
          ...
          <div class="flex flex-1 lg:flex-initial lg:min-w-[280px]">
            <label class="input input-sm w-full">
              ...
              <input id="busca" placeholder="Buscar usuários..." type="text" value="<?= esc($filtros['busca'] ?? '') ?>">
            </label>
          </div>
          <!-- Filtros -->
          <div class="flex flex-row gap-2.5 items-center">
            ...
            <select id="filtro_admin" class="select select-sm w-auto min-w-[140px]">
              <option value="">Todos os Tipos</option>
              <option value="1" <?= ($filtros['admin'] ?? '') == '1' ? 'selected' : '' ?>>Admin</option>
              <option value="0" <?= ($filtros['admin'] ?? '') == '0' ? 'selected' : '' ?>>Usuário</option>
            </select>
        ...
        <div data-datatable="true" data-datatable-page-size="10">
              ...
              <thead>
                <tr>
                  ...
                  <th class="min-w-[250px]">
                    <span class="sort">
                      <span class="sort-label font-normal text-gray-700">Usuário</span>
                  ...
                  <th class="min-w-[120px]">
                    <span class="sort">
                      <span class="sort-label font-normal text-gray-700">Telefone</span>
                      <span class="sort-icon"></span>
                    </span>
                  </th>
                  ...
                  <?php foreach ($usuarios as $user): ?>
                    <tr>
                      <td class="text-center">
                        <input class="checkbox checkbox-sm" data-datatable-row-check="true" type="checkbox" value="<?= $user['id_usuario'] ?>">
                      </td>
                      <td>
                        <div class="flex items-center gap-2.5">
                          ...
                          <div class="flex flex-col">
                            <a class="text-sm font-medium text-gray-900 hover:text-primary-active mb-px" href="<?= base_url('Usuarios/editar/' . $user['id_usuario']) ?>">
                              <?= esc($user['nome']) ?>
                            </a>
                          </div>
                        </div>
                      ...
                      <td>
                        <a class="text-sm text-gray-700 font-normal hover:text-primary-active" href="mailto:<?= esc($user['email']) ?>">
                          <?= esc($user['email']) ?>
                        </a>
                      </td>
                      ...
                      <td>
                        ...
                        <?php else: ?>
                          <span class="badge badge-sm">Usuário</span>
                        <?php endif; ?>
                      </td>
                      <td>
                        <div class="flex justify-center gap-2">
                          <a class="btn btn-sm btn-icon btn-light" href="<?= base_url('Usuarios/editar/' . $user['id_usuario']) ?>" title="Editar">
                            <i class="ki-filled ki-notepad-edit"></i>
                          </a>
                          <a class="btn btn-sm btn-icon btn-light-primary" href="<?= base_url('UsuarioPermissoes/atribuir/' . $user['id_usuario']) ?>" title="Permissões">
                            <i class="ki-filled ki-security-user"></i>
                          </a>
                          <a class="btn btn-sm btn-icon btn-light" href="#" 
                             onclick="return confirmarExclusao('<?= base_url('Usuarios/excluir/' . $user['id_usuario']) ?>', 'Excluir Usuário?', 'Tem certeza que deseja excluir este usuário? Esta ação não pode ser desfeita!')" title="Excluir">
          ...
          <div class="card-footer justify-center md:justify-between flex-col md:flex-row gap-5 text-gray-600 text-2sm font-medium">
            ...
            <div class="flex items-center gap-4 order-1 md:order-2">
              <span data-datatable-info="true">1-<?= count($usuarios) ?> de <?= count($usuarios) ?></span>
            </div>
          </div>
        </div>
      </div>
...
function aplicarFiltros() {
  ...
  const admin = document.getElementById('filtro_admin').value;
  ...
  window.location.href = '<?= base_url('Usuarios') ?>?' + params.toString();
}
