...
<footer class="footer">
        ...
        <div class="menu-dropdown menu-default w-full max-w-[175px]" data-menu-dismiss="true">
         ...
         <div class="menu-item" data-menu-item-offset="-15px, 0" data-menu-item-placement="right-start" data-menu-item-toggle="dropdown" data-menu-item-trigger="click|lg:hover">
          ...
          <div class="menu-dropdown menu-default w-full max-w-[175px]">
           <div class="menu-item">
            <a class="menu-link" href="html/demo6/account/home/settings-sidebar.html">
             <span class="menu-icon">
  ...
  <!-- jQuery (necessário para Select2) - verifica se já não está carregado -->
  <script>
  ...
  <script>
    // Funções auxiliares SweetAlert2
    window.SwalConfirm = function(title, text, callback, options = {}) {
    ...
    window.SwalAlert = function(title, text, icon = 'info', callback = null) {
      Swal.fire({
        title: title,
        text: text,
        icon: icon,
    ...
    window.SwalSuccess = function(title, text, callback = null) {
      Swal.fire({
        title: title || 'Sucesso!',
    ...
    window.SwalError = function(title, text, callback = null) {
      Swal.fire({
        title: title || 'Erro!',
        text: text || 'Ocorreu um erro ao processar a solicitação.',
        icon: 'error',
        confirmButtonText: 'OK'
      }).then((result) => {
        if (callback) {
    ...
    window.SwalWarning = function(title, text, callback = null) {
      Swal.fire({
        title: title || 'Atenção!',
        text: text || 'Por favor, verifique as informações.',
        icon: 'warning',
        confirmButtonText: 'OK'
      }).then((result) => {
        if (callback) {
    ...
    window.confirmarExclusao = function(url, titulo = 'Tem certeza?', texto = 'Esta ação não pode ser desfeita!') {
      Swal.fire({
        title: titulo,
        text: texto,
        icon: 'warning',
    ...
    // Exibe mensagens flash do servidor
    document.addEventListener('DOMContentLoaded', function() {
      // Verifica se há mensagens de sucesso/erro do servidor
      const sucessoMsg = document.querySelector('.alert-success');
      const erroMsg = document.querySelector('.alert-danger');
      
      if (sucessoMsg) {
        const texto = sucessoMsg.querySelector('span')?.textContent || sucessoMsg.textContent;
        if (texto && texto.trim()) {
          SwalSuccess('Sucesso!', texto.trim());
        }
      }
      
      if (erroMsg) {
        const texto = erroMsg.querySelector('span')?.textContent || erroMsg.textContent;
        if (texto && texto.trim()) {
          SwalError('Erro!', texto.trim());
        }
      }
    });
    
    // Inicializa Select2 em todos os selects com classe select2-select
    document.addEventListener('DOMContentLoaded', function() {
      // Inicializa Select2
      ...
      // Atualiza labels dos switches para mostrar apenas o estado atual
      function atualizarSwitchLabels() {
        document.querySelectorAll('.switch input[type="checkbox"]').forEach(function(checkbox) {
          const switchEl = checkbox.closest('.switch');
          if (!switchEl) return;
          
          const activeLabel = switchEl.querySelector('.switch-label-active');
          const inactiveLabel = switchEl.querySelector('.switch-label-inactive');
          
          if (!activeLabel || !inactiveLabel) return;
          
          // Remove qualquer estilo inline anterior
          activeLabel.removeAttribute('style');
          inactiveLabel.removeAttribute('style');
          
          if (checkbox.checked) {
            activeLabel.style.setProperty('display', 'inline-block', 'important');
            inactiveLabel.style.setProperty('display', 'none', 'important');
          } else {
            activeLabel.style.setProperty('display', 'none', 'important');
            inactiveLabel.style.setProperty('display', 'inline-block', 'important');
          }
        });
      }
      
      // Atualiza na inicialização (com delay para garantir que o DOM está pronto)
      setTimeout(atualizarSwitchLabels, 100);
      
      // Atualiza quando o checkbox muda
      document.addEventListener('change', function(e) {
        if (e.target.type === 'checkbox' && e.target.closest('.switch')) {
          atualizarSwitchLabels();
        }
      });
      
      // Atualiza quando o checkbox é clicado (para resposta mais rápida)
      document.addEventListener('click', function(e) {
        if (e.target.type === 'checkbox' && e.target.closest('.switch')) {
          setTimeout(atualizarSwitchLabels, 10);
        }
      });
      
      // Também atualiza quando a página termina de carregar
      window.addEventListener('load', function() {
        atualizarSwitchLabels();
      });
      
      // Observa mudanças no DOM para switches adicionados dinamicamente
      const observer = new MutationObserver(function(mutations) {
        mutations.forEach(function(mutation) {
          mutation.addedNodes.forEach(function(node) {
            if (node.nodeType === 1) {
              const switches = node.querySelectorAll ? node.querySelectorAll('.switch input[type="checkbox"]') : [];
              if (switches.length > 0 || (node.classList && node.classList.contains('switch'))) {
                setTimeout(atualizarSwitchLabels, 50);
              }
            }
          });
        });
    ...
    // Função para inicializar Select2 em elementos dinâmicos
