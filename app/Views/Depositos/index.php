  ...
  <div class="container-fixed flex items-center justify-between flex-wrap gap-3">
    <div class="flex items-center flex-wrap gap-1 lg:gap-5">
      ...
      <div class="flex items-center gap-1 text-sm font-normal">
        ...
        <span class="text-gray-900">Depósitos</span>
      </div>
    </div>
    <div class="flex items-center flex-wrap gap-1.5 lg:gap-3.5">
      <a class="btn btn-sm btn-primary" href="<?= base_url('Depositos/criar') ?>">
    ...
    <div class="card card-grid min-w-full">
      <div class="card-header flex flex-col lg:flex-row gap-4 lg:items-center lg:justify-between">
        <h3 class="card-title font-medium text-sm">
          Mostrando <?= count($depositos) ?> depósito(s)
        </h3>
        <div class="flex flex-col lg:flex-row gap-3 w-full lg:w-auto lg:items-center">
          ...
          <div class="flex flex-1 lg:flex-initial lg:min-w-[280px]">
            <label class="input input-sm w-full">
              ...
              <input id="busca" placeholder="Buscar depósitos..." type="text" value="<?= esc($filtros['busca'] ?? '') ?>">
            </label>
          </div>
          <!-- Filtros -->
        ...
        <div data-datatable="true" data-datatable-page-size="10">
              ...
              <thead>
                <tr>
                  ...
                  <th class="min-w-[200px]">
                    <span class="sort">
                      <span class="sort-label font-normal text-gray-700">Endereço</span>
                      <span class="sort-icon"></span>
                    </span>
                  </th>
                  <th class="min-w-[150px]">
                    <span class="sort">
                      <span class="sort-label font-normal text-gray-700">Responsável</span>
                      <span class="sort-icon"></span>
                    </span>
                  </th>
                  ...
                  <?php foreach ($depositos as $deposito): ?>
                    <tr>
                      <td class="text-center">
                        <input class="checkbox checkbox-sm" data-datatable-row-check="true" type="checkbox" value="<?= $deposito['id_deposito'] ?>">
                      </td>
                      <td>
                        <div class="flex items-center gap-2.5">
                          <div class="flex items-center justify-center size-8 rounded-full bg-primary/10 shrink-0">
                            <i class="ki-filled ki-archive text-primary"></i>
                          </div>
                          <div class="flex flex-col">
                            <a class="text-sm font-medium text-gray-900 hover:text-primary-active mb-px" href="<?= base_url('Depositos/editar/' . $deposito['id_deposito']) ?>">
                              <?= esc($deposito['nome']) ?>
                            </a>
                          </div>
                        </div>
                      ...
                      <td class="text-gray-800 font-medium">
                        <?= esc($deposito['endereco'] ?? '-') ?>
                      </td>
                      <td class="text-gray-800 font-medium">
                        <?= esc($deposito['responsavel'] ?? '-') ?>
                      </td>
                      <td class="text-gray-800 font-medium">
                        <?= esc($deposito['telefone'] ?? '-') ?>
                      </td>
                      <td>
                        <?php if ($deposito['ativo']): ?>
                      ...
                      <td>
                        <div class="flex justify-center gap-2">
                          <a class="btn btn-sm btn-icon btn-light" href="<?= base_url('Depositos/editar/' . $deposito['id_deposito']) ?>" title="Editar">
                            <i class="ki-filled ki-notepad-edit"></i>
                          </a>
                          <a class="btn btn-sm btn-icon btn-light" href="#" 
                             onclick="return confirmarExclusao('<?= base_url('Depositos/excluir/' . $deposito['id_deposito']) ?>', 'Excluir Depósito?', 'Tem certeza que deseja excluir este depósito? Esta ação não pode ser desfeita!')" title="Excluir">
          ...
          <div class="card-footer justify-center md:justify-between flex-col md:flex-row gap-5 text-gray-600 text-2sm font-medium">
            ...
            <div class="flex items-center gap-4 order-1 md:order-2">
              <span data-datatable-info="true">1-<?= count($depositos) ?> de <?= count($depositos) ?></span>
            </div>
          </div>
        </div>
      </div>
...
function aplicarFiltros() {
  ...
  window.location.href = '<?= base_url('Depositos') ?>?' + params.toString();
}
